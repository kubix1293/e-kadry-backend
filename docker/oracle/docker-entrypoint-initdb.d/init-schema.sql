            ...
            FOR i IN 1 .. iIlerat
                LOOP
                    ...
                    IF (lKwota >= lBuf AND i < iIlerat) THEN
                        ...
                        VALUES (lRata, iIdpkzppoz, to_char(to_date(lOks, 'YYYY-MM-DD HH24:MI:SS'), 'rrrr-mm'),lIdOks);
                        --
                        lOks := add_months(lOks, 1);
                        lBuf := lBuf + lRata;
                    ELSE
                        INSERT INTO pkzp_harm (kwot, id_pkzp, okres,id_oks)
                        VALUES (lRata + (lKwota - lBuf), iIdpkzppoz, to_char(to_date(lOks, 'YYYY-MM-DD HH24:MI:SS'), 'rrrr-mm'),lIdOks);
                        --
                        lOks := add_months(lOks, 1);
                        lBuf := lBuf + lRata;
                    END IF;
